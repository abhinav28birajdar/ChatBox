    ...
    const ThemeOption = ({ title, value, icon, desc }: any) => (
        ...
        >
            <View style={[styles.iconBox, { backgroundColor: colors.surface }]}>
                <Ionicons name={icon} size={28} color={selected === value ? colors.primary : colors.textSecondary} />
            </View>
            <View style={{ flex: 1, marginLeft: 16 }}>
                <Text variant="subtitle2" color={selected === value ? colors.primary : colors.text}>{title}</Text>
                <Text variant="caption" color={colors.textSecondary}>{desc}</Text>
            </View>
            {selected === value && <Ionicons name="checkmark-circle" size={24} color={colors.primary} />}
        </TouchableOpacity>
    );
